...
function SignInPage() {
  ...
  // Which OAuth providers are actually configured on the server. We only render
  // a social button when its provider exists — no dead buttons that 500.
  ...
  useEffect(() => {
    fetch("/api/auth/providers")
      .then((r) => r.json())
      .then((p) => setOauth({ google: !!p?.google, linkedin: !!p?.linkedin }))
      .catch(() => {});
  }, []);
    ...
    <div style={{
      ...
      <div style={{
        ...
        <p style={{ fontSize: 10, color: "#52525b", textAlign: "center", marginTop: 8, marginBottom: 16 }}>
          ...
          Returning? Use the same password.{oauth.google ? " Org owners can sign in with Google." : ""}
        </p>

        {/* ===== SOCIAL LOGIN — only render configured providers ===== */}
        {(oauth.google || oauth.linkedin) && (
          <>
            <div style={{ display: "flex", alignItems: "center", gap: 12, margin: "2px 0 14px" }}>
              <div style={{ flex: 1, height: 1, background: "#27272a" }} />
              <span style={{ fontSize: 10, color: "#52525b", textTransform: "uppercase", letterSpacing: 1 }}>or continue with</span>
              <div style={{ flex: 1, height: 1, background: "#27272a" }} />
            </div>
