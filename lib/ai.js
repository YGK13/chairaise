...
//
// Chai ($18) is the numerological heart of Jewish giving. A world-class ask
// engine doesn't just bolt $18 onto an arbitrary number (which produces noise
// like "$100,008"); it suggests the *resonant* chai figures major-gift officers
// actually use: chai, double/triple/quintuple chai, and their powers of ten —
// $18K, $36K, $54K, $72K, $90K, $180K, $360K ... These read clean AND carry the
// cultural signal that is ChaiRaise's entire thesis ("Multiply your impact by 18").
// ============================================================

// Curated ladder of resonant, recognizable chai-aligned ask amounts.
// Each rung is a classic chai multiple (1x/2x/3x/4x/5x) scaled by powers of ten.
export const CHAI_ASK_LADDER = [
  1800, 3600, 5400, 7200, 9000,
  18000, 36000, 54000, 72000, 90000,
  180000, 360000, 540000, 720000, 900000,
  1800000, 3600000, 5400000, 7200000, 9000000,
];

// Smaller, donor-facing chai gift tiers (used for suggested gift buttons).
export const CHAI_AMOUNTS = [18, 36, 54, 72, 90, 180, 360, 540, 1800, 3600, 5400, 10800, 18000, 36000, 54000];

// Snap an amount to the most resonant chai figure.
//  - At/above the ladder floor → nearest rung on the curated ladder (stays clean).
//  - Below the floor → nearest plain chai multiple ($18), where the $18 cadence
//    is still visible and meaningful to a donor.
export const roundToChai = (amount) => {
  const n = parseInt(amount || 0);
  if (n <= 0) return 0;
  if (n < CHAI_ASK_LADDER[0]) return Math.round(n / 18) * 18;
  return CHAI_ASK_LADDER.reduce(
    (best, rung) => (Math.abs(rung - n) < Math.abs(best - n) ? rung : best),
    CHAI_ASK_LADDER[0]
  );
};
...
export const aiAsk = (d) => {
  ...
  let base;
  // Net-worth tiers are themselves chai-aligned rungs so the common path is exact,
  // not lossy-snapped. Prior-giving path (1.5x last gift) snaps to the nearest rung.
  if (pg > 0) base = Math.max(pg * 1.5, 18000);
  else if (nw >= 1e8) base = 180000;  // 10x chai for nine-figure capacity
  else if (nw >= 5e7) base = 90000;   // 5x chai
  else if (nw >= 1e7) base = 54000;   // 3x chai
  else if (nw >= 5e6) base = 36000;   // 2x chai
  else base = 18000;                  // chai
